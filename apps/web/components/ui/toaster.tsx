						...
						<div className="flex w-full">
							<div className="space-y-2 w-full justify-center">
								<div className="flex space-x-2 justify-between">
									<div className="flex space-x-2 items-center">
										{props?.variant && (
											<div className="w-[20px] h-[20px] flex items-center">
												{props.variant === "ai" && <Icons.AIOutline className="text-[#0064D9]" />}
												{props?.variant === "success" && <Icons.Check />}
												{props?.variant === "error" && <Icons.Error className="text-[#FF3638]" />}
												{props?.variant === "progress" && <Icons.Spinner className="h-4 w-4 animate-spin" />}
												{props?.variant === "spinner" && <Icons.Spinner className="h-4 w-4 animate-spin" />}
											</div>
										)}
									...
									<div>
										{props?.variant === "progress" && <span className="text-sm text-[#878787]">{progress}%</span>}
									</div>
								</div>

								{props.variant === "progress" && (
									<Progress value={progress} className="w-full rounded-none h-[3px] bg-border" />
								)}
