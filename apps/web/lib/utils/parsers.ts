...
export function searchParamsCache<T extends Record<string, unknown>>(
	...
	return createSearchParamsCache({
		...
		sort: getSortingStateParser<T>().withDefault(defaultSort ? [defaultSort] : []),
	...
	return createParser({
		parse: (value) => {
			try {
				...
				if (validKeys && result.data.some((item) => !validKeys.has(item.column))) {
					return null;
				}
		...
		eq: (a, b) =>
			a.length === b.length &&
			a.every((item, index) => item.column === b[index]?.column && item.desc === b[index]?.desc),
	});
};
...
export type ExtendedColumnFilter<TData = unknown> = z.infer<typeof filterItemSchema>;
